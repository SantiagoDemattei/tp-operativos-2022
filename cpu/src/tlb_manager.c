...
void agregar(t_tlb* elemento_a_agregar){
        ...
        pthread_mutex_lock(&mutex_tlb_aux);
        pthread_mutex_unlock(&mutex_tlb_aux);

        if(list_size_con_mutex_tlb(tlb, mutex_tlb) < configuracion_cpu->entradas_tlb){ // si la tlb no esta llena ===> tiene espacio libre ===> agrego la entrada nueva
            list_add_con_mutex_tlb(tlb, elemento_a_agregar, mutex_tlb);
        }
        else { //si la tlb esta llena, tengo que reemplazar usando algun algoritmo
            ...
            case LRU:
                break;
            default:
                loggear_error(logger_cpu, "Algoritmo de reemplazo no reconocido");
                break;
            }
        }             
