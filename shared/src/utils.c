...
void loggear_tabla_pagina2(t_list *lista_tabla_segundo_nivel, t_log *logger, pthread_mutex_t mutex)
{
    ...
    log_info(logger, "       TABLA DE PAGINAS 2 NIVEL");

    for (int i = 0; i < list_size(lista_tabla_segundo_nivel); i++)
    {
        t_tabla_pagina2 *tabla_segundo_nivel = list_get(lista_tabla_segundo_nivel, i);
        log_info(logger, "Tabla id: %d\n", tabla_segundo_nivel->id_tabla);
        log_info(logger, " |  MARCO   | P  | U   | M |");
        for (int i = 0; i < list_size(tabla_segundo_nivel->segundo_nivel); i++)
        {
            t_estructura_2do_nivel *pagina2 = list_get(tabla_segundo_nivel->segundo_nivel, i);
            log_info(logger, " |  %d      |  %d  |  %d  |  %d  |", pagina2->marco, pagina2->presencia, pagina2->uso, pagina2->modificado);
        }
        log_info(logger, "---------------------\n");
    }
...
{ // busca el pcb en la cola y si no lo encuentra devuelve NULL
    int indice;
